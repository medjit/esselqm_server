...
//================= Audio Player ================
document.addEventListener('DOMContentLoaded', function () {
   // Get the audio file from the URL parameters
   const urlParams = new URLSearchParams(window.location.search);
   const audioFile = urlParams.get('file');

   if (audioFile) {
      // Fetch audio metadata and update the audio details in the DOM
      fetchAudioData(audioFile)
         .then((data) => {
            updateAudioDetails(data); // Update audio details (thumbnail, title, author, ID)
            setupDownloadButton(data.name); // Set up the download button functionality
            setupShareButton(audioFile); // Set up the share button functionality
         })
         .catch((error) => {
            console.error('Error:', error);
         });
      ...
      // Check and resume progress if available
      checkAndResumeProgress(audioPlayer, audioFile);

      // Save progress on timeupdate event
 ...
 * 
 * This function sends a GET request to the API endpoint to retrieve metadata for the specified audio file.
 * If the request is successful, it returns the parsed JSON data. If the request fails, it throws an error.
 * 
 * @async
 * @function fetchAudioData
 * @param {string} audioFile - The path to the audio file for which metadata is to be fetched.
 * @returns {Promise<Object>} The parsed JSON data containing the audio metadata.
 * @throws {Error} Throws an error if the fetch request fails.
 */
async function fetchAudioData(audioFile) {
   // Send a GET request to the API endpoint to retrieve metadata for the specified audio file
   const response = await fetch(
...
/**
 * Updates the audio details on the webpage.
 *
 * This function updates the audio thumbnail, title, author, and ID elements
 * on the webpage with the provided data.
 *
 * @param {Object} data - The data object containing audio details.
 * @param {Object} data.data - The nested data object containing image, title, and artist.
 * @param {string} data.data.image - The base64 encoded image data for the audio thumbnail.
 * @param {string} data.data.title - The title of the audio.
 * @param {string} data.data.artist - The artist of the audio.
 * @param {string} data.name - The file name of the audio, including the extension.
 */
function updateAudioDetails(data) {
   // Select the audio thumbnail element and update its source with the base64 image data
   const audioThumbnail = document.querySelector('.audio-thumbnail');
   audioThumbnail.src = `data:image/png;base64,${data.data.image}`;

   // Select the audio title element and update its text content with the audio title
   const audioTitle = document.querySelector('.audio-title');
   audioTitle.textContent = data.data.title;

   // Select the audio author element and update its text content with the audio artist
   const audioAuthor = document.querySelector('.audio-author');
   audioAuthor.textContent = data.data.artist;

   // Select the audio ID element and update its text content with the audio ID (file name without extension)
